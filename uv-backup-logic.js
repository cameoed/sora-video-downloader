...
(function initUVBackupLogic(globalScope) {
  ...
  function sameOwnerIdentity(left, right) {
    ...
    if (a.handle && b.handle && a.handle.toLowerCase() !== b.handle.toLowerCase()) return false;
  ...
  function extractOwnerIdentity(payload) {
    ...
    const owner = normalizeOwnerIdentity({
      ...
      id: profile?.user_id || profile?.id || profile?._id || root?.owner_id || root?.author_id || payload?.user_id || root?.shared_by,
    });
    return owner;
  }
